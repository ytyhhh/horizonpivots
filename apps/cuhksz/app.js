 (async () => {
  ...
  const officialHallOrder = [
    'serenity-lodge', 'lotus-pavilion', 'shaw-college-canteen', 'lakeview-terrace',
    'luna-marina', 'duan-family-canteen', 'haroma-cafeteria', 'mus-canteen',
    'le-stelle', 'coli-lofty', 'mcdonalds', 'lacesar-pizzaria'
  ]
  ...
  function filteredCourses() {
    ...
    return data.courses
      .filter((course) => !query || `${course.code}${course.name}${course.nameEn}${course.instructor}${course.description}`.toLowerCase().includes(query))
  ...
  function renderDining() {
    ...
    const halls = [...data.halls].sort((left, right) => {
      const leftOrder = officialHallOrder.indexOf(left.id)
      const rightOrder = officialHallOrder.indexOf(right.id)
    ...
    $('#page-dining').innerHTML = `
      ...
      <section class="dining-directory"><div class="section-heading"><div><h2>食堂与菜品</h2><p>12 处校内餐饮点已补充至目录。食堂营业和档口信息以现场为准。</p></div></div>${halls.length ? `<div class="hall-grid">${halls.map((hall, index) => `<article class="hall-card ${hall.tone}" data-open-type="hall" data-id="${hall.id}" tabindex="0" role="button"><div class="hall-number">食堂 ${String(index + 1).padStart(2, '0')}</div><h3>${escapeHTML(hall.name)}</h3>${hall.nameEn ? `<div class="hall-name-en">${escapeHTML(hall.nameEn)}</div>` : ''}<p>${escapeHTML(hall.location)}</p>${hall.description ? `<div class="hall-description">${escapeHTML(hall.description)}</div>` : ''}<footer><span>${escapeHTML(hall.hours)}</span><span>${hall.rating == null ? '暂无评分' : `★ ${hall.rating}`} · ${hall.stalls ? `${hall.stalls} 个档口` : '档口信息待补充'}</span></footer></article>`).join('')}</div>` : `<div class="empty-state"><div><b>食堂资料正在整理</b><p>暂未录入可展示的食堂和菜品数据。</p></div></div>`}<div class="section-heading dining-dishes-heading"><div><h2>${state.diningQuery ? '匹配的菜品' : '看看大家在吃什么'}</h2><p>${dishes.length} 道菜品，价格和供应情况可能随档口调整。</p></div></div>${dishes.length ? `<div class="dish-strip">${dishes.map(dishCard).join('')}</div>` : `<div class="empty-state"><div><b>没有找到相符菜品</b><p>当前搜索已优先展示校内服务。</p><button class="text-action" data-clear-dining>清除筛选</button></div></div>`}</section>`
  }
