  ...
  return (
    <section className="grid gap-7">
      <div className="max-w-2xl">
        <h1 className="m-0 text-3xl font-semibold tracking-[-0.035em] md:text-[42px] md:leading-[1.08]">{locale === "zh" ? "确认范围，然后开始检索" : "Confirm the scope, then start searching"}</h1>
        <p className="mt-4 max-w-[62ch] text-base leading-7 text-[var(--muted)]">{locale === "zh" ? "检索会并行分析所选学校，并实时更新学校级发现与官网核验进度。" : "Selected schools are processed in parallel with live school-level discovery and verification progress."}</p>
      </div>
      ...
      <div className="surface grid gap-4 p-5 md:grid-cols-[auto_1fr] md:items-start md:p-6">
        ...
        <div className="grid gap-2 text-sm leading-6 text-[var(--muted)]">
          <p className="m-0 font-semibold text-[var(--ink)]">{locale === "zh" ? "结果如何生成" : "How results are produced"}</p>
          <p className="m-0">{locale === "zh" ? "OpenAlex 负责发现候选及近期论文，Semantic Scholar 可补充论文摘要。Brave Search 在配置密钥后查找学校官方页面。硅基流动只根据提供的证据生成匹配解释，不会补造招生信息。" : "OpenAlex discovers candidates and recent work, while Semantic Scholar can enrich paper abstracts. Brave Search finds official school pages when configured. SiliconFlow explains fit from supplied evidence and never invents recruiting claims."}</p>
        </div>
      </div>

      <div className="flex items-center justify-between gap-3">
        <button className="button-secondary" disabled={loading} onClick={onBack}><ArrowLeft size={18} />{locale === "zh" ? "修改条件" : "Edit"}</button>
        <button className="button-primary min-w-36" disabled={loading} onClick={onStart}><MagnifyingGlass size={18} />{loading ? (locale === "zh" ? "正在创建任务" : "Starting") : (locale === "zh" ? "开始寻找导师" : "Find supervisors")}</button>
      </div>
    </section>
  );
}
